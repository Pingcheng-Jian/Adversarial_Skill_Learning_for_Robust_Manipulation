...
class MasspointPushSingleObstacleEnv(MasspointPushEnv, utils.EzPickle):
    ...
    def _reset_sim(self):
        ...
        if self.random_box and self.np_random.uniform() < self.random_ratio:
            ...
            while not config_valid(object_xpos, stick1_xpos):
                object_xpos = self.initial_masspoint_xpos[:2] + self.np_random.uniform(-self.obj_range, self.obj_range,
                                                                                       size=2)
                stick1_xpos = self.initial_masspoint_xpos[:2] + self.np_random.uniform(-self.obj_range, self.obj_range,
                                                                                       size=2)
        else:
    ...
    def _sample_goal(self):
        ...
        one_hot = np.zeros(2)
        one_hot[g_idx] = 1
        goal = self.initial_masspoint_xpos[:2] + self.target_offset + self.np_random.uniform(-self.target_range,
...
class MasspointPushSingleObstacleEnv_v2(MasspointPushEnv, utils.EzPickle):
    ...
    def _reset_sim(self):
        ...
        if self.random_pusher:
            masspoint_jointx_i = self.sim.model.get_joint_qpos_addr('masspoint:slidex')
            masspoint_jointy_i = self.sim.model.get_joint_qpos_addr('masspoint:slidey')
            masspoint_pos = self.initial_masspoint_xpos[:2] + self.np_random.uniform(-self.obj_range, self.obj_range,
                                                                                     size=2)
            while self.inside_wall(masspoint_pos):
                masspoint_pos = self.initial_masspoint_xpos[:2] + self.np_random.uniform(-self.obj_range,
        ...
        if self.random_box and self.np_random.uniform() < self.random_ratio:
            ...
            stick1_xpos = np.asarray(
                [np.random.choice([self.pos_wall0[0] + self.size_wall[0] + self.size_obstacle[0],
                                   self.pos_wall0[0] - self.size_wall[0] - self.size_obstacle[0]]),
                 self.initial_masspoint_xpos[1]])
    ...
    def _sample_goal(self):
        ...
        goal = self.initial_masspoint_xpos[:2] + self.target_offset + self.np_random.uniform(-self.target_range,
                                                                                             self.target_range, size=2)
        if hasattr(self, 'sample_hard') and self.sample_hard and g_idx == 0:
            while self.inside_wall(goal) or (goal[0] - self.pos_wall0[0]) * (
                    self.sim.data.get_site_xpos('object0')[0] - self.pos_wall0[0]) > 0:
                goal = self.initial_masspoint_xpos[:2] + self.target_offset + self.np_random.uniform(-self.target_range,
                                                                                                     self.target_range,
                                                                                                     size=2)
        else:
    ...
    def _reset_sim(self):
        ...
        if self.random_box and self.np_random.uniform() < self.random_ratio:
            ...
            while not config_valid(object_xpos, stick1_xpos, stick2_xpos):
                ...
                stick1_xpos = self.initial_masspoint_xpos[:2] + self.np_random.uniform(-self.obj_range, self.obj_range,
                                                                                       size=2)
                stick2_xpos = self.initial_masspoint_xpos[:2] + self.np_random.uniform(-self.obj_range, self.obj_range,
                                                                                       size=2)
        else:
            self.sample_hard = True
            ...
            if not self.sample_harder:
                while not (np.linalg.norm(object_xpos - masspoint_pos) >= 0.3
                           and abs(object_xpos[0] - self.pos_wall0[0]) >= self.size_object[0] + self.size_wall[0]
                           and abs(object_xpos[0] - self.pos_wall2[0]) >= self.size_object[0] + self.size_wall[0]
                           and (abs(object_xpos[0] - stick1_xpos[0]) >= self.size_object[0] + self.size_obstacle[
                            0] or abs(
                            object_xpos[1] - stick1_xpos[1]) >= self.size_object[1] + self.size_obstacle[1])
                           and (abs(object_xpos[0] - stick2_xpos[0]) >= self.size_object[0] + self.size_obstacle[
                            0] or abs(
                            object_xpos[1] - stick2_xpos[1]) >= self.size_object[1] + self.size_obstacle[1])):
                    object_xpos = self.initial_masspoint_xpos[:2] + self.np_random.uniform(-self.obj_range,
                                                                                           self.obj_range, size=2)
            else:
                while not (np.linalg.norm(object_xpos - masspoint_pos) >= 0.3
                           and abs(object_xpos[0] - self.pos_wall0[0]) >= self.size_object[0] + self.size_wall[0]
    ...
    def _sample_goal(self):
        ...
        if hasattr(self, 'sample_hard') and self.sample_hard:
            ...
            if hasattr(self, 'sample_harder') and self.sample_harder:
                ...
                while (same_side(goal[0], object_pos[0], self.pos_wall0[0]) and same_side(goal[0], object_pos[0],
                                                                                          self.pos_wall2[0])
                       or (same_side(goal[0], masspoint_pos[0], self.pos_wall0[0]) and same_side(goal[0],
                                                                                                 masspoint_pos[0],
                                                                                                 self.pos_wall2[0]))
...
class MasspointMazeEnv(MasspointPushEnv, utils.EzPickle):
    ...
    def _sample_goal(self):
        ...
        # one_hot = np.zeros(self.n_object)
        # one_hot[g_idx] = 1
        goal = np.array([2.5, 2.5]) + self.target_offset + self.np_random.uniform(-self.target_range, self.target_range,
                                                                                  size=2)
