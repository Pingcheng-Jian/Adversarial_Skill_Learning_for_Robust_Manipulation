...
from env_resource.envs.assets.fetch.single_generate_xml_stack import generate_xml
...
class FetchStackEnv(fetch_env.FetchEnv, utils.EzPickle):
    def __init__(self, reward_type='sparse', random_gripper=True, random_box=True, random_ratio=1.0, n_object=2):
        ...
        for i in range(n_object):
            initial_qpos['object%d:joint' % i] = [1.1 + 0.05 * i, 0.75 + 0.1 * (i - n_object // 2), 0.41, 1., 0., 0.,
                                                  0.]
    ...
    def _get_obs(self):
        ...
        if self.has_object:
            ...
            object_rel_pos = [object_pos[i] - grip_pos if i in self.selected_objects else np.zeros(3) for i in
                              range(self.n_object)]
            ...
            object_velp = [object_velp[i] - grip_velp if i in self.selected_objects else np.zeros(3) for i in
                           range(self.n_object)]
    ...
    def _reset_sim(self):
        ...
        if self.random_gripper:
            mocap_pos = np.concatenate([self.np_random.uniform([1.15, 0.6], [1.45, 0.9]), [0.6]])
    ...
    def _sample_goal(self):
        ...
        if self.task_mode == 1:
            if self.has_base:
                # base_nobject > 1
                goal = np.concatenate(
                    [self.maybe_goal_xy, [self.height_offset + self.size_object[2] * 2 * (self.current_nobject - 1)]])
                ...
                goal = np.concatenate([self.initial_gripper_xpos[:2] + self.np_random.uniform(
                    -self.target_range, self.target_range, size=2),
                                       [self.height_offset + self.size_object[2] * 2 * (self.current_nobject - 1)]])
                # g_idx = np.random.randint(self.current_nobject)
                g_idx = np.random.choice(self.selected_objects)
    ...
    def compute_reward_and_success(self, observation, goal, info):
        ...
        if obs_len == 2:
            ...
            achieved_goal = np.array([observation[i, 3 + 3 * idx[i]: 3 + 3 * (idx[i] + 1)] for i in range(len(idx))])
        else:
            task_mode = np.argmax(
                observation[observation_dim - 2: observation_dim])
            one_hot = goal[3:]
        ...
        if task_mode == 0:
            ...
            if self.reward_type == 'dense':
                ...
                if obs_len == 2:
                    ...
                    # Check if stacked
                    for i in range(T):
                        other_objects_pos = np.concatenate([observation[i][3: 3 + 3 * idx[i]], observation[i][3 + 3 * (
                                idx[i] + 1): 3 + 3 * self.n_object]])
                        stack = self._is_stacked(
                    ...
                    if r_achieve < -0.5:
                        ...
                        gripper_far = np.linalg.norm(
                            observation[:3] - achieved_goal) > 2 * self.distance_threshold
                        # print('stack', stack, 'gripper_far', gripper_far)
                        if stack and gripper_far:
                            r = 0.0
                        else:
    ...
    def step(self, action):
        ...
        intower_idx = list(filter(lambda idx: np.linalg.norm(
            obs['observation'][3 + 3 * idx: 3 + 3 * idx + 2] - obs['desired_goal'][:2]) < 0.025
                                              and abs((obs['observation'][3 + 3 * idx + 2] - self.height_offset)
                                                      - 0.05 * round(
            (obs['observation'][3 + 3 * idx + 2] - self.height_offset) / 0.05)) < 0.01,
            ...
            for prob_idx in intower_idx:
                if self.height_offset + 2 * self.size_object[2] * i < obs['desired_goal'][2] - 0.01:
                    ...
                    if prob_idx != np.argmax(obs['desired_goal'][3:]) and \
                            abs(obs['observation'][3 + 3 * prob_idx + 2] - (
                    ...
                    if abs(obs['observation'][3 + 3 * prob_idx + 2] - (
                            self.height_offset + 2 * self.size_object[2] * i)) < 0.01:
                        self.tower_height += 2 * self.size_object[2]
                        found = True
                        break
