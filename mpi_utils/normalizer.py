...
class normalizer:
    def __init__(self, size, eps=1e-2, default_clip_range=np.inf):
        ...
        self.default_clip_range_tensor = torch.tensor(default_clip_range)
    ...
    def normalize_tensor(self, v, clip_range=None, device='cpu'):
        if clip_range is None:
            clip_range = self.default_clip_range
        return torch.clamp((v - torch.tensor(self.mean).to(device)) / torch.tensor(self.std).to(device), -clip_range, clip_range)